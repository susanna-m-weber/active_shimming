...
coordinates = np.genfromtxt(posfilename, delimiter=',', skip_header=True)
...
#%%
def create_3d_matrix(coordinates, field_values):
    """
    Creates a 3D matrix from a list of coordinates and corresponding field values.    Args:
        coordinates (list of tuples): List of (x, y, z) coordinate tuples.
        field_values (list): List of field values corresponding to the coordinates.    Returns:
        numpy.ndarray: 3D matrix with field values at the specified coordinates.
    """    # Find the dimensions of the matrix
    x_coords, y_coords, z_coords = zip(*coordinates)
    x_dim = max(x_coords) + 1
    y_dim = max(y_coords) + 1
    z_dim = max(z_coords) + 1    # Create an empty 3D matrix
    matrix = np.zeros((int(x_dim), int(y_dim), int(z_dim)))    # Populate the matrix with field values
    for (x, y, z), value in zip(coordinates, field_values):
        matrix[x, y, z] = value    
...
ledger.to_excel('GA_original_50pop_250gens.xlsx', index=False)
ledger.to_csv('GA_original_50pop_250gens.csv')


# plt.figure()
# plt.scatter(res.F[0], res.F[1],s=100)
# plt.title("B0 vs. # coils used for GA optimization", fontsize=18, weight='bold')
# plt.xlabel("Standard deviation field (Hz)", fontsize=16, weight='bold')
# plt.ylabel("Number of coils", fontsize=16, weight='bold')
# plt.xticks(fontsize=14, weight='bold')
# plt.show()
...
B_loops_mT = B_loops*1000
b = B[:,0]*1000 - np.mean(B[:,0])*1000

# plot 3D shim field 
shim_field_3D, sensors  = shimsimulator.extract_3D_shim_field(loops,xmin=-76,xmax=76, ymin=-76,ymax=76,zmin=-76, zmax=76, numberpoints_per_ax = 55,Bcomponent=0) #(11, 11, 11, 3)
