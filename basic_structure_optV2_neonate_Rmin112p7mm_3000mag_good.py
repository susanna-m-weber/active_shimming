...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 18:17:24 2023
...
class MultiObjectiveMixedVariableProblem(ElementwiseProblem):
    ...
    def __init__(self, **kwargs):
        ...
        self.maxzpos = int(60)
        self.maxlayers = int(6)
        self.increment = int(self.maxzpos*self.maxlayers)
        
        self.cube_side_length = 25.4/2
        self.maxzextent = 1000
        self.maxnumberofmagnets = 2980
        self.rmin=112.7
        self.dr=np.sqrt(3)*self.cube_side_length+5
        
        # print(self.increment)
        variables = dict()

        variables["x01"] = Integer(bounds=(1, 4))
        variables["x02"] = Integer(bounds=(2, 30))
        
        for k in range(3, 8):
            variables[f"x{k:02}"] = Integer(bounds=(18, 24))
        ...
        for k in range(8+self.increment, 8+self.increment+self.increment//self.maxlayers):
            variables[f"x{k:02}"] = Integer(bounds=(8, 28))
        ...
        for k in range(8+self.increment*3, 8+self.increment*4):
            variables[f"x{k:02}"] = Real(bounds=(0, np.pi/16)) 
    ...
    def get_magnets(self,x):
        ...
        magnets = magpy.Collection(style_label='magnets')
        # print(tmp_df.shape)
        eta, meanB0,_,_ = magsimulator.simulate_ledger(magnets,col_sensors,mag_vect,tmp_df,0.06,4,True,False,None,False) 
        print('mean B0=' + str(np.round(meanB0,4)) + ' homogen=' + str(np.round(eta,4)))
...
filename = 'optimization_after_neonate_magnet_Rmin_112p7mm.xlsx'
